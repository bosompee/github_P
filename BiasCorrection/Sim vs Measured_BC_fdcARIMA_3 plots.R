...
#First, the residual series was obtained by subtracting the simulated flow from that of the observed.
df_corrected_with_obsC$diffC <- df_corrected_with_obsC$Q - df_corrected_with_obsC$SimCorrected
...
#converting the data first to time series
df_joinedtimeC=ts(df_corrected_with_obsC[,7],start=c(1980,10),frequency = 12)

#checking again if the dataset is a time series
class(df_joinedtimeC)  #data is in "ts" now
...
adf.test(df_joinedtimeC)

#################
SC = auto.arima(df_joinedtimeC, allowmean=FALSE, allowdrift=FALSE, trace=TRUE)
sC <- SC$residuals

dC <- cbind(df_corrected_with_obsC, sC)

d1C <- transform(dC, SimCorrectednewC= s+SimCorrected)

SimCorrectednew1C <- d1C $SimCorrectednewC                     # Duplicate data frame
SimCorrectednew1C[d1C $SimCorrectednewC  < 0] <- 0     # Set negative values to 0

#Add to dataframe
d1C <- cbind(d1C, SimCorrectednew1C)
...
dens <- ggplot(data=d1C, mapping=aes( x = Date))+
  geom_line(mapping = aes(y = SimCorrectednew1C, color= "SimCorrected"))+
  geom_line(mapping = aes(y = Sim, color= "Sim"))+     #change y value
  geom_line(mapping = aes(y = Q, color= "Q"))+
  scale_x_date(expand = c(0,0)) +
  #scale_x_continuous(breaks = seq(1980,2016,4))+
...
#First, the residual series was obtained by subtracting the simulated flow from that of the observed.
df_corrected_with_obsB$diffB <- df_corrected_with_obsB$Q - df_corrected_with_obsB$SimCorrected
...
#converting the data first to time series
df_joinedtimeB=ts(df_corrected_with_obsB[,7],start=c(1980,10),frequency = 12)

#checking again if the dataset is a time series
class(df_joinedtimeB)  #data is in "ts" now
...
adf.test(df_joinedtimeB)

#################
SB = auto.arima(df_joinedtimeB, allowmean=FALSE, allowdrift=FALSE, trace=TRUE)
sB <- SB$residuals

dB <- cbind(df_corrected_with_obsB, sB)

d1B <- transform(dB, SimCorrectednewB= s+SimCorrected)

SimCorrectednew1B <- d1B $SimCorrectednewB                     # Duplicate data frame
SimCorrectednew1B[d1B $SimCorrectednewB  < 0] <- 0     # Set negative values to 0

#Add to dataframe
d1C <- cbind(d1B, SimCorrectednew1B)
